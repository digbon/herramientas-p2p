...
export function AccountPicker({
  ...
  const options = store.accounts
    ...
    .filter((a) => {
      const platformNames =
        a.paymentMethods?.map((pm) => pm.label || pm.type).join(" ") || "";
      const platformValues =
        a.paymentMethods?.map((pm) => pm.value).join(" ") || "";
              ...
              <div className="flex flex-col items-start">
                <div className="flex items-center gap-2">
                  <span className="text-white font-bold text-sm tracking-tight">
                    {selectedAccount.platformValue || selectedAccount.name}
                  </span>
                  {selectedAccount.tag && (
                    <span className="text-[10px] bg-slate-800 text-slate-400 px-1.5 py-0.5 rounded font-black uppercase text-[8px]">
                      {selectedAccount.tag}
                    </span>
                  )}
                </div>
                {selectedAccount.ownerName && (
                  <span
                    className={cn(
                      ...
                      selectedAccount.ownerType === "Mias"
                        ? "text-purple-400"
                        : "text-teal-400",
                    )}
                  >
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex flex-wrap gap-1 mt-1">
                    {acc.paymentMethods?.map((pm) => (
                      <span
                        key={pm.id}
                        className="text-[9px] bg-slate-800 text-slate-400 px-1.5 py-0.5 rounded border border-slate-700/50 font-mono"
                      >
                        {pm.label || pm.type}: {pm.value}
                      </span>
                    ))}
                    {(!acc.paymentMethods ||
                      acc.paymentMethods.length === 0) && (
                      <span className="text-[9px] text-slate-600 italic">
                        Sin métodos enlazados
                      </span>
                    )}
                  </div>
                </div>
                    ...
                    <div className="flex flex-col items-end">
                      <span className="text-xs font-bold text-white tracking-tight">
                        {balance.toLocaleString(undefined, {
                          ...
                          maximumFractionDigits: acc.currency === "BTC" ? 5 : 2,
                        })}
                      </span>
